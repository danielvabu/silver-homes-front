...
class _TenantMaintenancScreenState extends State<TenantMaintenancScreen> {
  ...
  apimanager(String search, int pageNo, String SortField, int saquence, int ftime) async {
    TenantMaintenanceListReqtokens reqtokens = new TenantMaintenanceListReqtokens();
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Row(
                      children: [
                        if (tenantMaintenanceState!.isloding && tenantMaintenanceState.SearchText == "")
                          Container(
                            width: 260,
                            height: 30,
                            decoration: BoxDecoration(
                          ...
                          Container(
                            ...
                            child: Row(
                              children: <Widget>[
                                new Expanded(
                                  child: TextFormField(
                                    initialValue: tenantMaintenanceState.SearchText,
    ...
    return Container(
      ...
      child: PopupMenuButton(
        onSelected: (value) async {
          ...
          TenantMaintenanceListReqtokens reqtokens = new TenantMaintenanceListReqtokens();
          reqtokens.Applicant_ID = Prefs.getString(PrefsName.BT_ApplicantID);
          reqtokens.Name = MaintenanceState.SearchText != null ? MaintenanceState.SearchText : "";

          List<Sort> sortinglist = [];
          Sort sort = new Sort();
          if (MaintenanceState.SearchText != null && MaintenanceState.SearchText.isNotEmpty) {
            sort.fieldId = "ID";
            sort.sortSequence = 0;
          } else {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (MaintenanceState.maintenancedatalist != null && MaintenanceState.maintenancedatalist.length > 0) tablefooter(MaintenanceState)
        ],
      ),
    );
  }
    ...
    return Row(
      children: [
        MaintenanceState.isloding
            ...
            : MaintenanceState.maintenancedatalist != null && MaintenanceState.maintenancedatalist.length > 0
                ? Expanded(
                    child: TenantMaintenanceItem(
                          ...
                          builder: (BuildContext context1) {
                            return MaintenanceRequestDialogBox(
                              ...
                              onPressedEdit: (String mid) async {
                                ...
                                ApiManager().maintenanceDetailsApiCallback(context, mid, 2, (status, responce, details) async {
                                  if (status) {
                                    loader.remove();
                                    EditRequetsDialog();
                                  } else {
                              ...
                              showEdit: maintenanceData.IsLock != null ? !maintenanceData.IsLock! : true,
                      ...
                      onPresseEdit: (MaintenanceData maintenanceData, int pos) async {
                        ...
                        ApiManager().maintenanceDetailsApiCallback(context, maintenanceData.ID.toString(), 2,
                      ...
                      onPresseDuplicate: (MaintenanceData maintenanceData, int pos) {
                        showDialog(
                          context: context,
                          barrierColor: Colors.black45,
                          useSafeArea: true,
                          ...
                          builder: (BuildContext context1) {
                            return AlertDialogBox(
                              ...
                              onPressedYes: () {
                                Navigator.of(context1).pop();
                                duplicateMaintenance(maintenanceData, MaintenanceState);
                              },
                              onPressedNo: () {
                                Navigator.of(context1).pop();
                              },
                      ...
                      onPresseArchive: (MaintenanceData maintenanceData, int pos) {},
                    ),
                  )
                : Expanded(
                    child: Container(
        ...
        children: [
          ...
          Container(
            ...
            child: DropdownSearch<String>(
              ...
              hint: "Select page",
              defultHeight: Helper.PagingRecord(MaintenanceState.totalRecord).length * 35 > 250
                  ? 250
                  : Helper.PagingRecord(MaintenanceState.totalRecord).length * 35,
              selectedItem: MaintenanceState.pageNo.toString(),
              items: Helper.PagingRecord(MaintenanceState.totalRecord),
              showSearchBox: false,
              isFilteredOnline: true,
              onChanged: (value) {
                _store.dispatch(UpdateTenantMaintenance_pageNo(int.parse(value.toString())));
  ...
  createTableDataCSVFile(TenantMaintenanceState MaintenanceState) async {
    ...
    for (var data in MaintenanceState.maintenancedatalist.toSet()) {
      ...
      row.add(data.Date_Created != null && data.Date_Created != "0" && data.Date_Created != ""
          ? new DateFormat("dd-MMM-yyyy").format(DateTime.parse(data.Date_Created!)).toString()
          : "");
  ...
  addnewRequetsDialog(TenantMaintenanceState MaintenanceState) async {
    ...
    List<SystemEnumDetails> MaintenanceStatuslist = await QueryFilter().PlainValues(eSystemEnums().Maintenance_Status);
    List<SystemEnumDetails> MaintenanceCategorylist = await QueryFilter().PlainValues(eSystemEnums().Maintenance_Category);
  ...
  paginationCall(TenantMaintenanceState MaintenanceState, int pageno) {
    if (MaintenanceState.isRequestNameSort) {
      apimanager(MaintenanceState.SearchText, pageno, "RequestName", MaintenanceState.RequestNameSortAcsDes, 1);
    } else if (MaintenanceState.isCategorySort) {
      apimanager(MaintenanceState.SearchText, pageno, "Category", MaintenanceState.CategorySortAcsDes, 1);
    } else if (MaintenanceState.isPrioritySort) {
      apimanager(MaintenanceState.SearchText, pageno, "Priority", MaintenanceState.PrioritySortAcsDes, 1);
    } else if (MaintenanceState.isDateCreatedSort) {
      apimanager(MaintenanceState.SearchText, pageno, "Date_Created", MaintenanceState.DateCreatedSortAcsDes, 1);
    } else if (MaintenanceState.isCreatedBySort) {
      //apimanager(MaintenanceState.SearchText, pageno, "Type_User",MaintenanceState.CreatedBySortAcsDes, 1);
      apimanager(MaintenanceState.SearchText, pageno, "CreatedBy", MaintenanceState.CreatedBySortAcsDes, 1);
    } else if (MaintenanceState.isStatusSort) {
      apimanager(MaintenanceState.SearchText, pageno, "Status", MaintenanceState.StatusSortAcsDes, 1);
    } else if (MaintenanceState.isEditRightsSort) {
      apimanager(MaintenanceState.SearchText, pageno, "IsLock", MaintenanceState.EditRightsSortAcsDes, 1);
    } else {
  ...
  updateOtherSortingValue(int flag, TenantMaintenanceState MaintenanceState) {
    ...
    if (flag == 1) {
      _store.dispatch(UpdateTenantMaintenance_RequestNameSortAcsDes(MaintenanceState.RequestNameSortAcsDes == 1 ? 0 : 1));
      _store.dispatch(UpdateTenantMaintenance_CategorySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_PrioritySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_DateCreatedSortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_CreatedBySortAcsDes(0));
      ...
      apimanager("", 1, "RequestName", MaintenanceState.RequestNameSortAcsDes == 1 ? 0 : 1, 0);
    } else if (flag == 2) {
      _store.dispatch(UpdateTenantMaintenance_CategorySortAcsDes(MaintenanceState.CategorySortAcsDes == 1 ? 0 : 1));
      _store.dispatch(UpdateTenantMaintenance_RequestNameSortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_PrioritySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_DateCreatedSortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_CreatedBySortAcsDes(0));
      ...
      apimanager("", 1, "Category", MaintenanceState.CategorySortAcsDes == 1 ? 0 : 1, 0);
    } else if (flag == 3) {
      _store.dispatch(UpdateTenantMaintenance_PrioritySortAcsDes(MaintenanceState.PrioritySortAcsDes == 1 ? 0 : 1));
      _store.dispatch(UpdateTenantMaintenance_RequestNameSortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_CategorySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_DateCreatedSortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_CreatedBySortAcsDes(0));
      ...
      apimanager("", 1, "Priority", MaintenanceState.PrioritySortAcsDes == 1 ? 0 : 1, 0);
    } else if (flag == 4) {
      _store.dispatch(UpdateTenantMaintenance_DateCreatedSortAcsDes(MaintenanceState.DateCreatedSortAcsDes == 1 ? 0 : 1));
      _store.dispatch(UpdateTenantMaintenance_RequestNameSortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_CategorySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_PrioritySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_CreatedBySortAcsDes(0));
      ...
      apimanager("", 1, "Date_Created", MaintenanceState.DateCreatedSortAcsDes == 1 ? 0 : 1, 0);
    } else if (flag == 5) {
      _store.dispatch(UpdateTenantMaintenance_CreatedBySortAcsDes(MaintenanceState.CreatedBySortAcsDes == 1 ? 0 : 1));
      _store.dispatch(UpdateTenantMaintenance_RequestNameSortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_CategorySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_PrioritySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_DateCreatedSortAcsDes(0));
      ...
      apimanager("", 1, "CreatedBy", MaintenanceState.CreatedBySortAcsDes == 1 ? 0 : 1, 0);
    } else if (flag == 6) {
      _store.dispatch(UpdateTenantMaintenance_StatusSortAcsDes(MaintenanceState.StatusSortAcsDes == 1 ? 0 : 1));
      _store.dispatch(UpdateTenantMaintenance_RequestNameSortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_CategorySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_PrioritySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_DateCreatedSortAcsDes(0));
      ...
      apimanager("", 1, "Status", MaintenanceState.StatusSortAcsDes == 1 ? 0 : 1, 0);
    } else if (flag == 7) {
      _store.dispatch(UpdateTenantMaintenance_EditRightsSortAcsDes(MaintenanceState.EditRightsSortAcsDes == 1 ? 0 : 1));
      _store.dispatch(UpdateTenantMaintenance_RequestNameSortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_CategorySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_PrioritySortAcsDes(0));
      _store.dispatch(UpdateTenantMaintenance_DateCreatedSortAcsDes(0));
      ...
      apimanager("", 1, "IsLock", MaintenanceState.EditRightsSortAcsDes == 1 ? 0 : 1, 0);
    }
  }
  ...
  duplicateMaintenance(MaintenanceData maintenanceData, TenantMaintenanceState MaintenanceState) {
    loader = Helper.overlayLoader(context);
    Overlay.of(context)!.insert(loader);

    ApiManager().duplicateMaintenaceworkflow(context, maintenanceData.ID.toString(), "2", (error, respoce) async {
      if (error) {
        //loader.remove();
        init();
        ...
        if (respoce != null && respoce.isNotEmpty) {
          ...
          ApiManager().maintenanceDetailsApiCallback(context, respoce, 2, (status, responce, details) async {
            if (status) {
              //loader.remove();
