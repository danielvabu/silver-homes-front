...
import 'package:silverhome/widget/landlord/action_popup/activetenant_popupmenu.dart';

import '../../alert/message_dialogbox.dart';
...
class ActiveTenantItem extends StatefulWidget {
  ...
  @override
  _ActiveTenantItemState createState() => _ActiveTenantItemState();
}

class _ActiveTenantItemState extends State<ActiveTenantItem> {
  ...
  void initState() {
    ...
    leasereviewstatuslist = QueryFilter().PlainValuesWithSorting(eSystemEnums().LeaseStatus);
  ...
  List<Widget> _tableData(TenancyApplication model, int Index) {
    ...
    result.add(_datavalueLeaseDuration(""));
    result.add(_appstatusdropdown(model));
    result.add(_priviewLease(model));
  ...
  Widget _datavalueLeaseStartDate(String text) {
    return Container(
      height: 40,
      width: width / 9,
      padding: EdgeInsets.only(left: 10),
  ...
  Widget _datavalueLeaseDuration(String text) {
    return Container(
      height: 40,
      width: width / 9,
      padding: EdgeInsets.only(left: 10),
      ...
      child: DropdownSearch<SystemEnumDetails>(
        ...
        onChanged: (data) {
          if (model.applicationStatus != null &&
              model.applicationStatus!.EnumDetailID.toString() == eApplicationStatus().ActiveTenent.toString()) {
            _dailogRemoveActiveTenant(model, data!.EnumDetailID.toString());
          } else if (data!.EnumDetailID.toString() != eApplicationStatus().ActiveTenent.toString()) {
            TenancyApplicationID updateid = new TenancyApplicationID();
  ...
  Widget _priviewLease(TenancyApplication model) {
    return (model.agreementReceivedDate != null && model.agreementReceivedDate != "")
        ? InkWell(
            onTap: () {
            ...
            child: Container(
              ...
              child: Text(
                GlobleString.ATH_Preview_Lease,
  ...
  Widget _actionPopup(TenancyApplication model) {
    return Expanded(
      ...
      child: Container(
        ...
        child: ActiveTenantPopupMenu(
          isListView: true,
          onPressedACArchive: () {
            _dailogTerminanteTenant(model);
            // _dailogSetArchiveActiveTenant(model);
          },
          onPressedACEditTenet: () {
            CustomeWidget.EditApplicant(context, model.applicantId.toString());
          },
          onPressedInviteTenant: () {
            _dailogTenantInvite(model);
          },
        ),
      ),
    );
      ...
      builder: (BuildContext context1) {
        return AlertDialogBox(
          ...
          onPressedYes: () {
            ...
            ApiManager().CheckTenantActiveOrNot(context, model.propId.toString(), model.applicantId.toString(), (status, responce) async {
              if (status) {
                await activetenantcallApi();
      ...
      builder: (BuildContext context1) {
        return AlertDialogBox(
          ...
          onPressedYes: () {
            ...
            if (Prefs.getString(PrefsName.user_CustomerFeatureListingURL).isNotEmpty) {
              tenantRegister(model);
            } else {
              _dailogErrorTenant(model);
              //ToastUtils.showCustomToast(context, GlobleString.Plz_add_companydetails_defore_invite_tenant, false);
            }
          },
          onPressedNo: () {
            Navigator.of(context1).pop();
  ...
  _dailogErrorTenant(TenancyApplication model) async {
    showDialog(
      barrierColor: Colors.black45,
      context: context,
      useSafeArea: true,
      ...
      builder: (BuildContext context1) {
        return AlertDialogBox(
          title: GlobleString.Plz_add_companydetails_defore_invite_tenant,
          negativeText: GlobleString.Plz_add_companydetails_GotoProfile,
          positiveText: GlobleString.Plz_add_companydetails_OK,
          onPressedYes: () {
            Navigator.of(context1).pop();
          },
          onPressedNo: () {
  ...
  tenantRegister(TenancyApplication model) {
    ...
    ApiManager().landlord_ProfileDSQCall(context, Prefs.getString(PrefsName.OwnerID), (error, respoce2, landlordProfile) {
      if (error) {
        ...
        if ((landlordProfile!.CustomerFeatureListingURL != null && landlordProfile.CustomerFeatureListingURL!.isNotEmpty)) {
          String url = Weburl.CustomerFeaturedPage +
              landlordProfile.CustomerFeatureListingURL! +
              "/" +
          ...
          String CompanyName;
          if (landlordProfile.companyname != null && landlordProfile.companyname!.isNotEmpty) {
            CompanyName = landlordProfile.companyname!;
          } else {
            CompanyName = GlobleString.App_Name;
          }
          ...
          ApiManager().tenantRegisterWorkflow(
              ...
              CompanyLogo,
              url,
              model.applicantName!.split(" ").first,
              landlordProfile.firstname!,
              landlordProfile.lastname!,
              CompanyName, (status, responce) async {
            if (status) {
              loader.remove();
  ...
  terminateTenant(TenancyApplication model) {
    ...
    ApiManager().terminateTenancyWorkflow(context, model.applicantId.toString(), (status, responce) async {
      if (status) {
        loader.remove();
        ToastUtils.showCustomToast(context, GlobleString.ARC_terminante_successfully, true);
        activetenantcallApi();
      } else {
        loader.remove();
        ToastUtils.showCustomToast(context, responce, false);
