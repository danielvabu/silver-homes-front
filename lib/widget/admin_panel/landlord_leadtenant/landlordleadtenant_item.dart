...
typedef VoidCallDetails = void Function(LeadTenantData data);

class LandLordLeadTenantItem extends StatefulWidget {
  List<LeadTenantData> listdata;
  final VoidCallDetails _callbackDetails;
  final VoidCallDetails _callbackPropertyDetails;

  LandLordLeadTenantItem({
  ...
  @override
  _LandLordLeadTenantItemState createState() => _LandLordLeadTenantItemState();
}

class _LandLordLeadTenantItemState extends State<LandLordLeadTenantItem> {
  ...
  List<Widget> _tableData(LeadTenantData model, int Index) {
    ...
    result.add(_datavalueLandlordProperty(model));
    ...
    return Container(
      ...
      child: Text(
        model.applicationStatus != null
            ? model.applicationStatus!.displayValue
            : "",
        textAlign: TextAlign.center,
        style: MyStyles.Medium(12, myColor.Circle_main),
      ),
  ...
  Widget _datavalueLandlordProperty(LeadTenantData model) {
    return InkWell(
      onTap: () {
