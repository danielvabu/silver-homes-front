...
class SelectionWidget<T> extends StatefulWidget {
  ...
  static final ValueNotifier<bool> isCheckedNotifier = ValueNotifier(false);
...
class _SelectionWidgetState<T> extends State<SelectionWidget<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      constraints: BoxConstraints(
          maxHeight: widget.isMultiSelectionMode
              ? (widget.maxHeight! + 50)
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          if (widget.items!.length > 0) _multiSelectionValidation(),
        ],
      ),
    );
  }
  ...
  Widget _multiSelectionValidation() {
    ...
    isSelectAll = widget.items!.length == _selectedItems.length;
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          ...
          children: [
            Padding(
              padding: const EdgeInsets.only(bottom: 10, left: 10, right: 8),
              child: Wrap(
                  ...
                  SizedBox(
                    ...
                    child: Checkbox(
                      ...
                      onChanged: (value) {
                        ...
                        if (value!) {
                          _selectedItems.addAll(List.from(widget.items!));
                        }
            ...
            Padding(
              ...
              child: ElevatedButton(
                style: ButtonStyle(
                  backgroundColor:
                      MaterialStateProperty.all(myColor.Circle_main),
                ),
  ...
  updatedata(List<T> _syncItems, bool value) {
    _addDataToStream(widget.selectedValues);
  ...
  void _handleSelectedItem(T newSelectedItem) {
    if (widget.isMultiSelectionMode) {
      if (_selectedItems.contains(newSelectedItem)) {
        ...
        if (isSelectAll != (widget.items!.length == _selectedItems.length))
