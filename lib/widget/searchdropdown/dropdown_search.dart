...
import 'dropdownclass/modal_dialog.dart';
import 'dropdownclass/popupMenu.dart';
import 'dropdownclass/popup_safearea.dart';
import 'dropdownclass/scrollbar_props.dart';
import 'dropdownclass/selection_widget.dart';
import 'dropdownclass/text_field_props.dart';

export 'dropdownclass/popup_safearea.dart';
export 'dropdownclass/scrollbar_props.dart';
export 'dropdownclass/text_field_props.dart';
...
typedef ValueRemove<T> = void Function(T value);
...
class DropdownSearch<T> extends StatefulWidget {
  ...
  ///set a custom color for the focuscolor
...
class DropdownSearchState<T> extends State<DropdownSearch<T>> {
      ...
      builder: (context, data, wt) {
        return IgnorePointer(
          ...
          child: TextButton(
            ...
            //focusNode: widget.focusScopeNode != null ? FocusScopeNode() : widget.focusScopeNode,
            child: _formField(),
          ),
        );
      },
  ...
  Widget _defaultSelectedItemWidget() {
    Widget defaultItemMultiSelectionMode(T? item) {
      return Container(
        ...
        child: TextFormField(
          initialValue: _selectedItemAsString(item),
  ...
  Widget _defaultSelectedItemWidgetnew(FormFieldState state) {
    Widget defaultItemMultiSelectionMode(T? item) {
      return InkWell(
        onTap: () {
        ...
        return ValueListenableBuilder<bool>(
            ...
            builder: (context, isFocused, w) {
              return InputDecorator(
                ...
                isFocused: isFocused,
                decoration: _manageDropdownDecoration(state),
                child: _defaultSelectedItemWidgetnew(state),
              );
            });
      },
    );
            ...
            focusedBorder: OutlineInputBorder(
              borderSide: BorderSide(
                  color: widget.focuscolor != null
                      ? widget.focuscolor!
                      : myColor.Circle_main,
                  width: widget.focusWidth != null ? widget.focusWidth! : 1),
            ),
            enabledBorder: OutlineInputBorder(
              borderSide: BorderSide(
                  color: widget.isError!
                      ? widget.errorcolor!
                      : widget.enablecolor != null
                          ? widget.enablecolor!
                          : myColor.gray,
                  width: 1.0),
            ),
          ...
          CustomPopupMenuItem(
            child: Container(
              ...
              child: _selectDialogInstance(
                  defaultHeight: widget.defultHeight ?? 350),
            ),
          ),
        ]);
  }
  ...
  Widget _selectDialogInstance({double? defaultHeight}) {
    return SelectionWidget<T>(
      ...
      ttextstyle: widget.textstyle ?? MyStyles.Medium(14, myColor.Circle_main),
    );
  }
